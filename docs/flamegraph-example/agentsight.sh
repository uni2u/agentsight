...
TAG_RULES=(
  ...
  --tag-rule 'prompt:review=(?i)review|审核|check|看看|diff|pr'
  ...
  --tag-rule 'prompt:view=(?i)看看|视图|维度|排列|火焰图|llm call'
  ...
  --tag-rule 'llm:code=(?i)代码|函数|实现|返回|参数|调用|执行|struct|impl|变量|定义'
  --tag-rule 'llm:edit=(?i)修改|修复|继续|处理|让我|更新|添加|删除|移除|替换'
  --tag-rule 'llm:review=(?i)分析|检查|验证|问题|看|确认|发现|检测|注意'
  --tag-rule 'llm:explain=(?i)解释|说明|意思|表示|理解|是指|这是|这个|因为|所以'
  --tag-rule 'llm:suggest=(?i)建议|可以|应该|需要|推荐|尝试|使用|考虑'
  --tag-rule 'llm:confirm=(?i)好的|明白|了解|完成|已经|成功|没有|正确'
  --tag-rule 'llm:list=(?i)以下|如下|列表|步骤|方法|首先|然后|接下来|最后'
  --tag-rule 'llm:git=(?i)commit|push|merge|branch|rebase|git|版本'
  --tag-rule 'llm:test=(?i)测试|test|cargo|运行|通过|失败|错误'
  --tag-rule 'llm:file=(?i)文件|目录|路径|创建|读取|写入|保存'
  --tag-rule 'llm:query=(?i)你|我|请|如果|怎么|为什么|什么|哪'
  --tag-rule 'llm:action=(?i)执行|运行|启动|停止|安装|配置|设置'
  --tag-rule 'llm:result=(?i)结果|输出|返回|得到|生成|产生'
  --tag-rule 'llm:state=(?i)状态|当前|目前|现在|之前|之后'
  --tag-rule 'llm:tool=(?i)工具|命令|脚本|程序|系统'
)
