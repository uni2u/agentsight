...
- Projection：从同一事实表生成不同 stack 的方法。例如 system projection 关注
  ...
  prompt tag。
- Operation：语义剖析模型中的统一 profile frame。Session、prompt、LLM call、tool call、
  child process、file write、network connect 和 status 都可以表示成不同 kind 的
  operation。详见 [../semantic-operation-profiling.md](../semantic-operation-profiling.md)。
- Weighted operation tree：`agentpprof` 当前实现使用的最小 IR。每个 operation 只有
  parent、kind、name 和 value；parent 是 v0 唯一 relation，value 是当前 profile 的
  宽度单位。
...
更一般的模型在
[../semantic-operation-profiling.md](../semantic-operation-profiling.md) 中定义：raw events
先映射成 weighted operation tree，再把所有 value 大于 0 的 operation path 折叠成
profile。本文档评估的是这个模型在 AgentFlame/agentpprof 当前实现中的一个受限实例。
...
工程 backend 设计在
[../agentpprof-backend-architecture.md](../agentpprof-backend-architecture.md) 中展开。
本文的实验已经覆盖其中一部分路径：Codex/Claude ingest、semantic tagging、lineage、
system/token/duration operation trees、pprof/folded/SVG/JSON artifacts 和 package/readback
smokes。尚未完整覆盖的是 operation-tree rewrite backend、embedding/LLM backend、
profile diff 和 critical-path attribution。
...
后续论文版本应把 baseline 分成三类，而不是只列单个对照：

| Group | Baseline | 目的 |
| --- | --- | --- |
| Naive unit | raw prompt、prompt hash、session ID、LLM/tool call、tool name、span name | 证明自然可见单位要么高基数，要么太粗/太低层 |
| Semantic unit | regex tags、coding taxonomy、embedding clustering、local LLM tags | 比较不同 semantic operation 后端的覆盖率、稳定性和可解释性 |
| Effect-only | process command、file path、network domain、system effect type | 证明系统事实可靠但缺少上游语义时很难解释“为什么” |

当前 R170/R224/R223/R251 已经覆盖 flat/nonsemantic/session/prompt 相关 baseline，
但还没有系统比较 embedding、LLM、taxonomy 和规则后端。

### Backend 质量指标

为了比较不同 operation backend，后续实验应统一报告：

- coverage：多少 events、tokens、time 或 effects 被分配到 operation；
- unknown rate：多少被归到 `unknown`；
- cardinality：operation labels 和 frame values 是否过多；
- compression ratio：raw events/spans/prompts 到 unique profile paths 的压缩比例；
- stability：跨天、跨项目、跨 agent 的 label distribution drift；
- purity：有人工标注时的 label/cluster 混杂程度；
- top-k explainability：top-k operations 能解释多少 token/time/effect mass；
- backend cost：运行时间、内存、LLM 调用成本；
- privacy：profile 是否泄露 raw prompt、raw response、secret、完整路径或私有 URL。

其中 cardinality 和 privacy 应作为 hard gates。高基数字段应进入 sidecar JSON 或 raw
drilldown，不应默认进入 folded stack frame。
...
3. 做 backend baseline study。至少比较 raw prompt、session-only、tool-only、effect-only、
   regex、taxonomy、embedding clustering 和 local LLM tagging，报告 coverage、cardinality、
   compression、stability、top-k explainability、backend cost 和 privacy gates。
4. 扩展 C4 network boundary。重点是 Codex/Claude-launched target-network rows 的 orphan/
   missing-action cases，而不是再重复 direct Python positive controls。
5. 做 external-machine artifact run。验证 `agentpprof` 从 clean install 到 pprof/flamegraph
   output 的路径，在另一台机器和真实但可公开 sanitization policy 下可复现。
6. 明确默认产品体验。默认输出应少而有用：pprof `.pb.gz`、folded stack、SVG 和 summary
