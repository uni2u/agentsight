...
**最简单的方式 —— `exec` 直接启动你的智能体并自动追踪（零配置）：**

```bash
# 把你的智能体命令写在 `exec --` 后面即可。AgentSight 会自动发现要 hook 的二进制
# （解析符号链接 / 包装脚本）、推导进程过滤器、启动智能体，并在其退出时停止追踪。
sudo ./agentsight exec -- claude
sudo ./agentsight exec -- claude -p "总结这个仓库"
...
`exec` 是追踪智能体最简单的方式。把要运行的命令写在 `exec --` 后面，其余的 AgentSight 全部自动处理：
...
# 适用于任何智能体：命令照常书写即可
sudo ./agentsight exec -- claude -p "审查我的最后一次提交"
sudo ./agentsight exec -- python my_agent.py
...
> **`sudo` 提示**：在 `sudo` 下，`exec` 仍会找到*你自己*的用户级安装
> （它会读取 `$SUDO_USER` 的主目录下的 `~/.local/bin`、`~/bin` 和 `~/.nvm`），
> 因此 `sudo ./agentsight exec -- claude` 追踪的是你主目录里的 claude，而不是 root `$PATH` 上的其他版本。
...
`exec` 和 `record` 默认启动 Web UI。低层 `debug trace` 需要传入 `--server`：
